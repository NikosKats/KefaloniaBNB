...
export const POST: APIRoute = async ({ request, redirect, locals }) => {
  ...
  if (!id || !label || !chat_id) return redirect('/admin/telegram?msg=error');

  const service = getServiceClient();
  const { error } = await service
    .from('telegram_channels')
    .update({ label, chat_id, owner_id, notify_booking_new, notify_booking_confirmed, notify_booking_cancelled })
    .eq('id', id);
