...
export const POST: APIRoute = async ({ request, locals }) => {
  ...
  if (!file || !technicianId) return new Response(JSON.stringify({ error: 'file and technician_id required' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
  ...
  const storageKey = `technicians/${technicianId}/${crypto.randomUUID()}.${ext}`;
  ...
  const { data: tech } = await service.from('technicians').select('cover_photo, photo_urls').eq('id', technicianId).single();
  if (!tech) return new Response(JSON.stringify({ error: 'Technician not found' }), { status: 404, headers: { 'Content-Type': 'application/json' } });

  const updates: Record<string, unknown> = {};
  if (isCover || !tech.cover_photo) {
    updates.cover_photo = publicUrl;
  }
  if (!isCover) {
    const existing = Array.isArray(tech.photo_urls) ? tech.photo_urls : [];
    updates.photo_urls = [...existing, publicUrl];
  }
  updates.updated_at = new Date().toISOString();

  await service.from('technicians').update(updates).eq('id', technicianId);
